  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-3 w-full sm:w-auto">
        {/* Navigation & Today */}
        <div className="flex items-center gap-1 bg-surface-container-low p-1 rounded-xl border border-outline-variant/40 shrink-0">
          <button
            type="button"
            onClick={onPrevMonth}
        ...
        <button
          ...
          className="px-3.5 sm:px-5 py-2.5 rounded-full bg-primary text-on-primary text-xs font-semibold hover:bg-primary/90 transition-all shadow-sm hover:shadow-md flex items-center gap-1.5 sm:gap-2 cursor-pointer shrink-0"
        >
          <span className="material-symbols-outlined text-base">add</span>
          <span>Schedule <span className="hidden sm:inline">Interview</span></span>
        </button>
      </div>
    </div>
  );
